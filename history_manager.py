...
class HistoryManager:
    ...
    def record_single_exercise(self, exercise):
        ...
        workout_record = {
            "date": datetime.now().isoformat(), # Đã trả lại .isoformat() để khớp chuẩn String
            "exercises": [exercise] 
        }
        try:
            self.collection.insert_one(workout_record)
    ...
    def _load_and_aggregate_history(self):
        ...
        try:
            # Truy vấn các buổi tập trong 7 ngày gần nhất, sắp xếp tăng dần theo ngày
            ...
            for workout in recent_workouts:
                ...
                for ex in workout.get("exercises", []):
                    ...
                    if target not in aggregated:
                        # FIX LỖI 2: Khai báo sẵn biến last_trained_timestamp
                        aggregated[target] = {"hits_this_week": 0, "last_trained_timestamp": workout_date}
                    
                    aggregated[target]["hits_this_week"] += 1
                    # Cập nhật liên tục để giữ lại ngày tập gần nhất (mới nhất)
